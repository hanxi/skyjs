...
const SUITE = [
    ...
    { name: "fs_exit", config: "test/config-fs-exit.json", timeoutMs: 60000,
        mustRe: [/FS_EXIT_OK streamed=\d+ heap_delta=-?\d+ checks=\d+/] },
    { name: "crypto_node", config: "test/config-crypto-node.json",
        mustRe: [/CRYPTO_NODE_OK hash=1 hmac=1 random=1 zlib=1 checks=\d+/] },
    { name: "http_node", config: "test/config-http-node.json", timeoutMs: 30000,
        mustRe: [/HTTP_NODE_OK request=1 post=1 404=1 fetch=1 https=1 checks=\d+/] },
    { name: "native_switch", config: "test/config-native-off.json",
        mustRe: [/NATIVE_SWITCH_OK enabled=[01] checks=\d+/] },
    { name: "native_ext", config: "test/config-native-ext.json",
        mustRe: [/NATIVE_EXT_OK load=1 abi=1 crc32=1 resolve=1 checks=\d+/] },
    { name: "net", config: "test/config-net.json", timeoutMs: 30000,
        mustRe: [/NET_OK server=1 client=1 tls=1 checks=\d+/] },
    { name: "subprocess_switch", config: "test/config-subprocess-off.json",
        mustRe: [/SUBPROCESS_SWITCH_OK enabled=[01] checks=\d+/] },
    { name: "subprocess", config: "test/config-subprocess.json", timeoutMs: 60000,
        mustRe: [/SUBPROCESS_OK spawn=1 exec=1 execfile=1 stdin=1 kill=1 named=1 checks=\d+/] },
    { name: "fs_owner", config: "test/config-fs-owner.json",
        mustRe: [/FS_OWNER_OK binary=1 permission=3 checks=\d+/] },
    { name: "fs", config: "test/config-fs.json",
        mustRe: [/FS_OK sync=1 callback=1 promise=1 stream=1 errors=1 checks=\d+/] },
